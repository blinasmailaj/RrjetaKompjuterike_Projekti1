import socket
from _thread import *
from datetime import datetime
import random
import hashlib,os, binascii
import platform


serverName = ''
serverPort = 14000
serverSocket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
try:
    serverSocket.bind((serverName, serverPort))
except socket.error as e:
    print(str(e))

print('Serveri eshte ekzekutuar ne 127.0.0.1:' + str(serverPort))
serverSocket.listen(5)
print('Server is ready and waiting for commands')


def IP():
    return str(address[0])


def NRPORTIT():
    return str(address[1])


def NUMERO(text):
    vowels = ['A', 'a', 'e', 'E', 'i', 'I', 'o', 'O', 'u', 'U', 'y', 'Y']
    v = 0
    c = 0
    for i in str(text):
        if i in vowels:
            v += 1
        elif (i >= 'a' and i <= 'z') or (i > 'A' and i <= "Z"):
            c += 1

    return "Teksti ka " + str(v) + " zanore dhe " + str(c) + " bashketingellore"


def LOJA():
    arr = (random.sample(range(1, 35), 5))
    arr.sort()
    return str(arr)


def ANASJELLTAS(str):
    s = str.strip()
    reversed = ""
    indeksi = len(s)
    while indeksi > 0:
        reversed += s[indeksi - 1]
        indeksi = indeksi - 1
    return reversed


def PALINDROM(text):
    if (text == text[::-1]):
        return "Fjala " + text.lower() + " eshte palindrom "
    else:
        return "Fjala " + text.lower() + " nuk eshte palindrom "


def GCF(a, b):
    if a % b == 0:
        return b
    return GCF(b, a % b)


def KONVERTO(s, gjatesia):

    s=s.lower()
    if (s == "cmtoinch"):
        return round(gjatesia * 0.393701,2)
    elif (s == "inchtocm"):

        return round(gjatesia * 2.54,2)
    elif (s == "kmtomiles"):

        return round(gjatesia * 0.621371,2)
    elif (s == "milestokm"):
        return round(gjatesia * 1.60934,2)
    else:
        return "Lloji i konvertimit eshte gabim. Termat e sakte : cmtoinch, inchtocm ,kmtomiles dhe milestokm"



def HASHPWD(password):
   
    salt = hashlib.sha256(os.urandom(60)).hexdigest().encode('ascii')
    pwdhash = hashlib.pbkdf2_hmac('sha512', password.encode('utf-8'), 
                                salt, 100000)
    pwdhash = binascii.hexlify(pwdhash)
    string="Salted Hash Paswword "
    return {string: (salt + pwdhash).decode('ascii')}
 

def PLATFORM():

    platformdetails = platform.platform()
    systemname = platform.system()
    proccesorname = platform.processor()
    architecturedetails = platform.architecture()

    return{'platform details':platformdetails,
           'system name':systemname,
           'proccesor name':proccesorname,
           'architecture details':architecturedetails}
    
def sendRequest(request, conn, addr):
    requestarray = request.upper()
    requestarray = requestarray.split(" ")
    kerkesa1 = request.split(" ")
    if (requestarray[0] == 'IP'):
        if len(requestarray) == 1:
            conn.send(str.encode('IP Adresa e klientit eshte:' + IP()))
        else:
            conn.send(str.encode("Metoda IP nuk ka parametra"))
    elif (requestarray[0] == 'NRPORTIT'):
        if len(requestarray) == 1:
            conn.send(str.encode("Klienti eshte duke perdorur portin " + NRPORTIT()))
        else:
            conn.send(str.encode("Metoda NRPORTIT nuk ka parametra"))
    elif (requestarray[0] == 'NUMERO'):
        if len(requestarray) == 1:
            conn.send(str.encode("Metoda Numero ka argument tekstin pas saj"))
        else:
            s = requestarray[1:]
            conn.send(str.encode(str(NUMERO(s))))
    elif (requestarray[0] == 'ANASJELLTAS'):
        if len(requestarray) == 1:
            conn.send(str.encode("Metoda Anasjelltas ka argument tekstin pas saj"))
        else:
            s = request[len(requestarray[0]):]
            conn.send(str.encode((ANASJELLTAS(str(s)))))
    elif (requestarray[0] == 'GCF'):
        if len(requestarray) == 1:
            conn.send(str.encode("Metoda GCF duhet te permbaje 3 argumente"))
        elif len(requestarray) == 3:
            if requestarray[1].isdigit() and requestarray[2].isdigit():
                nr1 = int(requestarray[1])
                nr2 = int(requestarray[2])
                conn.send(str.encode(str(GCF(nr1, nr2))))
            else:
                conn.send(str.encode("Metoda GCF duhet te permbaje ne 2 argumentet e fundit vetem numra"))
        else:
            conn.send(str.encode("Metoda GCF duhet te kete 2 numra si argumenta"))
    elif (requestarray[0] == 'PALINDROM'):
        if len(requestarray) != 2:
            conn.send(str.encode("Metoda Palindrom ka argument tekstin pas saj"))
        else:
            s = requestarray[1]
            k = PALINDROM(s)
            conn.send(str.encode(str(k)))
    elif (requestarray[0] == 'KOHA'):
        if len(requestarray) == 1:
            time = datetime.now().strftime('%d.%m.%Y %H:%M:%S %p')
            conn.send(str.encode(time))
        else:
            conn.send(str.encode("Metoda Koha nuk ka parametra"))
    elif (requestarray[0] == 'LOJA'):
        if len(requestarray) == 1:
            conn.send(str.encode(str(LOJA())))
        else:
            conn.send(str.encode("Metoda Loja nuk ka parametra"))
    elif requestarray[0] == 'KONVERTO':
        convert = "Konvertimet :\ncmToInch  \ninchToCm  \nkmToMiles \nMileToKm";
        try:
            s = kerkesa1[1]
            n = float(kerkesa1[2])
            conn.send(str.encode(str(KONVERTO(s, n))))
        except IndexError:
            conn.send(str.encode(
                "Shenoni fillimisht llojin e konvertimit dhe me pas vleren \n" + convert))
        except ValueError:
            conn.send(str.encode(
                "Shenoni fillimisht llojin e konvertimit dhe me pas vleren \n" + convert))
    elif requestarray[0]=='HASHPWD':
        if len(requestarray) == 2:
            s=kerkesa1[1]
            conn.send(str.encode(str(HASHPWD(s))))
        else:
            conn.send(str.encode("Shtypni passwordin pas metodes HASHPWD"))
    elif requestarray[0]=='PLATFORM':
        if len(requestarray) == 1:
            conn.send(str.encode(str(PLATFORM())))
        else:
            conn.send(str.encode("Metoda Platform nuk ka parametra"))

    else:
        conn.send(str.encode("Nuk keni shtypur asnje nga kerkesat e dhena!"));


def clientThreading(conn, addr):
    while True:
        try:
            data = conn.recv(1024);
            request = data.decode();
            requestarray = request.split();
            try:
                sendRequest(request, conn, addr);
            except IndexError:
                conn.send(str.encode("Kerkesa nuk eshte valide!"))
        except OSError:
            conn.close();



while True:
    connectionSocket, address = serverSocket.accept()
    print('Klienti eshte lidhur me serverin ne %s  ne portin %s' % address)
    start_new_thread(clientThreading, (connectionSocket, address))



